...
runautostart(void)
{
	...
	if (access(path, X_OK) == 0) {
		log_info("runautostart running '%s' ...", path);
		system(path);
		log_info("runautostart running '%s' ... done", path);
	}
...
applyrules(Client *c)
{
	...
	c->preserveOnRestart = true;
	c->underCurrentIfFloat = false;
	...
	for (i = 0; i < LENGTH(rules); i++) {
		...
		&& (!r->instance || strstr(instance, r->instance)))
		{
			...
			c->tags |= r->tags;
			c->preserveOnRestart = r->preserveOnRestart;
			c->underCurrentIfFloat = r->underCurrentIfFloat;
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		i = x = 0;
		/** current layout symbol text width */
		int blw = TEXTW(selmon->ltsymbol);
		do
			x += TEXTW(tags[i]);
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		/* 2px right padding */
		else if (ev->x > selmon->ww - (int)TEXTW(stext) - getsystraywidth() - 2)
			click = ClkStatusText;
		else {
			x += blw;
			c = m->clients;
...
{
	unsigned int i, n, h, mw, mx, my, oty, ety, tw, nn;
	Client *c;
...
centeredfloatingmaster(Monitor *m)
{
	...
	if (n > m->nmaster) {
		...
		if (m->ww > m->wh) {
			mw = m->nmaster ? m->ww * m->mfact : 0;
			mh = m->nmaster ? m->wh * 1 : 0;
		} else {
			mh = m->nmaster ? m->wh * m->mfact : 0;
			mw = m->nmaster ? m->ww * 1 : 0;
		}
		mx = mxo = (m->ww - mw) / 2;
		my = myo = (m->wh - mh) / 2;
	} else {
...
void drawWin(Monitor *m)
{
	...
	m->winwin = XCreateWindow(
		dpy, root, posX, posY + bh, w, h, 0, DefaultDepth(dpy, screen),
		CopyFromParent, DefaultVisual(dpy, screen),
		CWOverrideRedirect | CWBackPixmap | CWEventMask, &wa);

	XDefineCursor(dpy, m->winwin, cursor[CurNormal]->cursor);
	XMapRaised(dpy, m->winwin);

	drw_setscheme(drw, scheme[SchemeNorm]);
	drw_text(drw, 0, 0, w, h, 0, "", 0);

	for (i = 0; i < LENGTH(tags); i++) {
		int y = 0;
		int tagiw = TEXTW(tags[i]);
		for (c = m->clients; c; c = c->next) {
			if (!ISINTAG(c, i)) {
				continue;
			}
			int ix = x + (tagiw - ICONSIZE) / 2;
			int iy = y + (bh - ICONSIZE) / 2;
			if (c->icon) {
				drw_pic(drw, ix, iy, c->icw, c->ich, c->icon);
				y += bh;
			}
		}
		x += tagiw;
...
void winEnd()
{
	...
	for (m = mons; m; m = m->next) {
		if (m->isWin) {
			/* turn off/destroy the window */
			XUnmapWindow(dpy, m->winwin);
			XDestroyWindow(dpy, m->winwin);
			m->isWin = 0;
		}
	}
}
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - getsystraywidth() - x) > bh) {
		if (n > 0) {
			int remainder = w % n;
...
key_release(XEvent *e)
{
	...
	// log_info("c key_release keycode=%u, state=%u", ev->keycode, ev->state);

	if (ev->keycode == 133 && ev->state == 64) {
		winEnd();
	}
}
...
keypress(XEvent *e)
{
	...
	// log_info("c keypress keycode=%u, state=%u", ev->keycode, ev->state);

	if (ev->keycode == 133 && ev->state == 0) {
		winStart();
	}
}
...
manage(Window w, XWindowAttributes *wa)
{
	...
	if (c->underCurrentIfFloat && selmon->sel) {
		c->x = selmon->sel->x + (selmon->sel->w / 2) - (c->w / 2);
		c->y = selmon->sel->y + (selmon->sel->h / 2) - (c->h / 2);
	}
	setclienttagprop(c);
